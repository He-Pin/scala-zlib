...
def fix_file(path):
    ...
    # 4. do-while replacement for Scala 3
    # Use: while ({ body; cond }) ()
    # BUT we must handle multiline and ensure semicolon before the block if it follows an expression.
    ...
    while True:
        ...
        # To avoid being treated as a param block, we use a semicolon or newline.
        # Actually, while ({ ... }) () is safe if preceded by ;
        new_loop = f"; while ({{ {body}; {cond} }}) ()"

        content = content[:start_do] + new_loop + content[end_while_paren+1:]
        pos = start_do + len(new_loop)

    # Special case: do {} while (cond)
    content = re.sub(r';\s*while\s*\((.*?)\)\s*\{\s*\}', r'; while (\1) {}', content)
